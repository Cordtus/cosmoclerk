...
export function checkAndHandleSessionExpiration(ctx: Context): boolean {
  ...
  if (!userId) {
    console.error(
      `[${new Date().toISOString()}] Error: Unable to retrieve user ID from context.`,
    );
    return false;
  }
  ...
  if (userAction) {
    ...
    if (
      ...
      ctx.reply(
        'Your session has expired due to inactivity. Please start a new session by using /start.',
      );
      return true;
    }
  }
