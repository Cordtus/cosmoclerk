import { getUserLastAction, updateUserLastAction, UserAction } from "./userSessions";  // Import UserAction
...
export function resetUserActionState(userId: number, fields: Array<keyof UserAction>): void {
  ...
  if (userAction) {
    ...
    console.error(`[${new Date().toISOString()}] No action found for user ${userId} to reset.`);
  }
}
