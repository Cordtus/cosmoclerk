...
export async function isEndpointHealthy(
  endpoint: string,
  isRpc: boolean,
): Promise<boolean> {
  ...
  if (endpoint.startsWith('http://') || unhealthyEndpoints.has(endpoint)) {
    console.log(
      `Skipping health check for endpoint with non-secure protocol or known to be unhealthy: ${endpoint}`,
    );
    return false;
  }
        ...
        .get(url, (resp) => {
          ...
          resp.on('end', () => {
            try {
              ...
              const latestBlockTimeString = isRpc
                ? jsonData.result?.sync_info?.latest_block_time
                : jsonData.block?.header?.time;
              const latestBlockTime = latestBlockTimeString
                ? new Date(latestBlockTimeString)
                : null;
              resolve(
                latestBlockTime
                  ? new Date().getTime() - latestBlockTime.getTime() < 60000
                  : false,
              );
            } catch {
              unhealthyEndpoints.add(endpoint);
