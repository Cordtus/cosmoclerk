...
export async function resetSessionAndShowChains(ctx: Context): Promise<void> {
  ...
  try {
    console.log(
      `[${new Date().toISOString()}] Restarting session for user: ${userId}`,
    );
    ...
    console.error(
      `[${new Date().toISOString()}] Error resetting session for user ${userId}:`,
      error,
    );
    await ctx.reply(
      'An error occurred while resetting your session. Please try again.',
    );
  }
}
