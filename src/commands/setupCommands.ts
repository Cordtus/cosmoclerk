...
import { getUserSettings, updateUserSettings } from "../sessionManager/settingsManager";
...
export function setupCommands(bot: Telegraf): void {
  ...
  bot.command("lookup", (ctx) => {
    ...
    if (parameter) {
      ...
      ctx.reply("Please provide a chain name to lookup. Example: /lookup cosmos");
    }
  });
  ...
  bot.command("testnet", (ctx) => {
    ...
    if (parameter) {
      ...
      ctx.reply("Please provide a testnet chain name. Example: /testnet cosmos-testnet");
    }
  });
  ...
  bot.command("validators", (ctx) => {
    ...
    if (parameter) {
      ...
      ctx.reply("Please provide a chain name to get validators. Example: /validators cosmos");
    }
  });
  ...
  bot.command("governance", (ctx) => {
    ...
    if (parameter) {
      ...
      ctx.reply("Please provide a chain name to get governance details. Example: /governance cosmos");
    }
  });
  ...
  bot.command("network", (ctx) => {
    ...
    if (parameter) {
      ...
      ctx.reply("Please provide an RPC URL to get network status. Example: /network https://rpc.cosmos.network");
    }
  });
...
bot.command("addchain", (ctx) => {
  ...
  if (parameter) {
    ...
    ctx.reply("Please provide a chain name to add to your list. Example: /addchain cosmos");
  }
});
...
bot.command("removechain", (ctx) => {
  ...
  if (parameter) {
    ...
    ctx.reply("Please provide a chain name to remove from your list. Example: /removechain cosmos");
  }
});
...
bot.command("setnotifications", async (ctx) => {
  ...
  if (parameter === "on" || parameter === "off") {
    ...
    updateUserSettings(userId, { ...currentSettings, notifications: parameter === "on" });
    await ctx.reply(`Notifications have been turned ${parameter}.`);
  } else {
    await ctx.reply("Please specify 'on' or 'off'. Example: /setnotifications on");
  }
});
}
