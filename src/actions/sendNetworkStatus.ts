...
export async function sendNetworkStatus(ctx: Context, rpcUrl: string): Promise<void> {
  ...
  try {
    ...
    console.error(`[${new Date().toISOString()}] Error sending network status for RPC ${rpcUrl}:`, error);
    await ctx.reply('An error occurred while fetching network status. Please try again.');
  }
}
