...
export async function sendValidatorsDetails(
  ctx: Context,
  chain: string,
): Promise<void> {
  ...
  try {
    ...
    console.error(
      `[${new Date().toISOString()}] Error sending validators for ${chain}:`,
      error,
    );
    await ctx.reply(
      'An error occurred while fetching validator details. Please try again.',
    );
  }
}
