...
} from '../botUtils/cacheUserPreferences';
...
export async function listUserChains(ctx: Context): Promise<void> {
  ...
  try {
    ...
    console.error(
      `[${new Date().toISOString()}] Error listing preferred chains for user ${userId}:`,
      error,
    );
    await ctx.reply(
      'An error occurred while trying to list your preferred chains. Please try again.',
    );
  }
}
