...
export function setupActions(bot: Telegraf<Context>): void {
  ...
  bot.on('callback_query', async (ctx) => {
    try {
      ...
      console.error(`[${new Date().toISOString()}] Error in callback query:`, error);
    }
  });
  ...
  bot.command('chainaction', async (ctx) => {
    ...
    if (args && args.length > 2) {
      ...
      await ctx.reply("Please provide a valid chain name and action type for the action.");
    }
  });
  ...
  bot.command('mychains', async (ctx) => {
    try {
      ...
      console.error(`[${new Date().toISOString()}] Error listing user chains:`, error);
    }
  });
