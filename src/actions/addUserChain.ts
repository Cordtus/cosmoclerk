...
export async function addUserChain(ctx: Context, chain: string): Promise<void> {
  ...
  try {
    ...
    if (userAction) {
      ...
      updateUserLastAction(userId, {
        customData: { preferredChains: [chain] },
      });
      await ctx.reply(`Chain "${chain}" has been added to your list.`);
    }
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Error adding chain ${chain} for user ${userId}:`,
      error,
    );
    await ctx.reply(
      'An error occurred while trying to add the chain. Please try again.',
    );
  }
}
