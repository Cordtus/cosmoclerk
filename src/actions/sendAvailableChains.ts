...
export async function sendAvailableChains(
  ctx: Context,
  ...
  try {
    ...
    console.error(
      `[${new Date().toISOString()}] Error sending available chains to user ${userId}:`,
      error,
    );
    await ctx.reply(
      'An error occurred while fetching the list of available chains. Please try again.',
    );
  }
}
