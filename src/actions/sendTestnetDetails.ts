...
export async function sendTestnetDetails(
  ctx: Context,
  chain: string,
): Promise<void> {
  ...
  try {
    ...
    console.error(
      `[${new Date().toISOString()}] Error sending testnet details for ${chain}:`,
      error,
    );
    await ctx.reply(
      'An error occurred while fetching testnet details. Please try again.',
    );
  }
}
