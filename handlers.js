...
function registerHandlers(bot) {
    bot.command('testnets', async (ctx) => {
        ...
        const chains = await getChainList('testnets');
        const keyboardMarkup = paginateChains(chains, 0, userId, config.pageSize);
    ...
    bot.action(/^select_chain:(.+)$/, async (ctx) => {
        ...
        updateUserLastAction(userId, { chain: chain, browsingTestnets: browsingTestnets });
        const keyboardMarkup = await sendMainMenu(ctx, userId);
    ...
    bot.on('text', async (ctx) => {
        ...
        if (text === '/start' || text === '/restart') {
            ...
            if (chains.map(chain => chain.toLowerCase()).includes(text)) {
                updateUserLastAction(userId, { chain: text, browsingTestnets: browsingTestnets });
                const keyboardMarkup = await sendMainMenu(ctx, userId);
                await ctx.reply('Select an action:', keyboardMarkup);
            } else {
    ...
    bot.action('peer_nodes', async (ctx) => {
        ...
        if (userAction && userAction.chain) {
            try {
                const peerNodes = await chainPeerNodes(ctx, userAction.chain);
                if (!peerNodes.trim()) {
                    throw new Error('Peer nodes information is currently unavailable.');
                }
                await editOrSendMessage(ctx, userId, peerNodes, { parse_mode: 'Markdown', disable_web_page_preview: true });
            } catch (error) {
    ...
    bot.action('block_explorers', async (ctx) => {
        ...
        if (userAction && userAction.chain) {
            try {
                const blockExplorers = await chainBlockExplorers(ctx, userAction.chain);
                if (!blockExplorers.trim()) {
                    throw new Error('Block explorer information is currently unavailable.');
                }
                await editOrSendMessage(ctx, userId, blockExplorers, { parse_mode: 'Markdown', disable_web_page_preview: true });
            } catch (error) {
